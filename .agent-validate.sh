...
status=$?
if [ "$status" -eq 0 ]; then
  lake build Mathlib.Topology.Algebra.Module.Equiv \
    Mathlib.Analysis.Calculus.ContDiff.Operations >> .agent-build.log 2>&1
  status=$?
fi
...
if [ "$status" -eq 0 ]; then
  rm -f .agent-build.log .agent-patch.py .agent-validate.sh \
    .github/workflows/agent-apply-continuous-linear-equiv-neg.yml
  git add -A
